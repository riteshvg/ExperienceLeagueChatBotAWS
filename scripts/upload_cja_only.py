#!/usr/bin/env python3
"""
Upload Customer Journey Analytics Documentation to S3

This script uploads only the Customer Journey Analytics documentation
from the English repository to S3.
"""
...
class CJAUploader:
    """Handles uploading Customer Journey Analytics documentation to S3."""
    
    def __init__(self):
        """Initialize the CJA uploader."""
        self.setup_logging()
    ...
    def list_uploaded_docs(self) -> bool:
        ...
        try:
            response = self.s3_client.list_objects_v2(
                ...
                Prefix="adobe-docs/customer-journey-analytics/"
            )
            
            if 'Contents' in response:
                self.logger.info(f"Found {len(response['Contents'])} CJA documents in S3:")
                ...
                self.logger.info("No CJA documents found in S3")
    ...
    def run_upload(self) -> bool:
        """Run the CJA document upload process."""
        self.logger.info("Starting Customer Journey Analytics documentation upload...")
        ...
        try:
            ...
            if self.upload_cja_docs():
                ...
                self.logger.error("❌ Failed to upload Customer Journey Analytics docs")
                return False
            
            # List uploaded documents
            self.list_uploaded_docs()
            
            self.logger.info("🎉 CJA document upload completed successfully!")
            return True
...
def main():
    """Main function."""
    try:
        uploader = CJAUploader()
        success = uploader.run_upload()
        
        if success:
            print("\n✅ CJA document upload completed successfully!")
            sys.exit(0)
        else:
            print("\n❌ CJA document upload failed!")
            sys.exit(1)
